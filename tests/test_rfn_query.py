...
from bgbb_airflow.sql_utils import run_rec_freq_spk, rec_freq_spk2pandas, to_s3_fmt
...
def clients_daily_df():
    ...
    client_dates_i = [
        ...
        ("Before_beginning", [0]),  # Not included, since this is before model range
        ("Halfway_start", [5, 6, 11]),
        ("Every_day", list(range(11))),
...
def test_user_active_before_range_not_in_rfn(rfn):
    ...
    # to RFN data
    assert (
        rfn.loc["Beginning_end", rfn_cols] == rfn.loc["Beginning_end_extra", rfn_cols]
    ).all()
...
def test_date_range(rfn, clients_daily_df):
    ...
    clients_daily_dates = pd.to_datetime(clients_daily_df.submission_date_s3).dt.date
...
def test_rfn_invariants(rfn, duration=None):
    ...
    max_dur = rfn.eval("Max_day - Min_day").astype("timedelta64[D]").astype(int).max()
